...
# APP_HOME is where the script is located
...
# Determine the Java 9+ module-path if needed.
if [ -f "$APP_HOME/gradle/wrapper/gradle-wrapper.properties" ]; then
    if grep -q "^distributionUrl=.*\.zip" "$APP_HOME/gradle/wrapper/gradle-wrapper.properties"; then
        CLASSPATH="$CLASSPATH"
    fi
fi
